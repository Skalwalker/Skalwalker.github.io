        ...
        <Accordion.Item eventKey="2">
          ...
          <Accordion.Body className="text-center">
            <ProjectParagraph>
              ...
              position and shape. The spectral centroid is the center of &quot;gravity&quot; of the
    ...
    <ProjectPageContainer project={projectContent.projects.covid} footer={footerInfo}>
      <ProjectSection key="intro-header" id="intro-header" title="Introduction" variant="first">
        <ProjectParagraph>
          ...
          approaches: a Support Vector Machine (SVM) and a Convolutional Neural Network (CNN) model.
          While the SVMs have been widely used and achieved outstanding results in traditional audio
          classification tasks, the Convolutions Neural Networks have gained space approaching the
          problem as an image to be classified [3]. This simple approach would provide a
          pre-screening method where every person with a microphone could be able to test for the
          disease.
        </ProjectParagraph>
      </ProjectSection>
      <ProjectSection key="dev-header" id="dev-header" title="Approach">
        <ProjectParagraph>
          C19-Audit takes advantage of two approaches: an SVM and a CNN model. On the one hand, SVMs
          have been widely used and achieved outstanding results in traditional audio classification
          tasks using standard audio features [4]. On the other hand, the Convolutions Neural
          Networks have gained space in audio classification tasks, using time-frequency
          representations such as the spectrograms and the Mel-spectrograms as an image to be
          classified [3]. Therefore, C19-Audit was designed to evaluate two different{' '}
        ...
        <ProjectParagraph>
          ...
          16-bit audio files, normalize them, and fixes their minimum length to a predetermined
          value. If the audio is smaller than the minimum length, zero padding is applied to the
          end. Hence, we have a fixed-length waveform. This waveform can be used to compute the
        ...
        <ProjectParagraph>
          ...
          convenient to process sounds [5]. The Mel spectrogram, the magnitude spectrogram, and the
          waveforms were used to calculate multiple audio features.
        </ProjectParagraph>
      </ProjectSection>
      <ProjectSection key="feature-header" id="features-header" title="Features">
        <ProjectParagraph>
          Using the signal representations, I generated a set of time, frequency, and Mel frequency
          features. I used all generated features&apos; statistics for the SVM to train the model,
          while for the CNN, the log Mel Spectrogram is set as an input feature. Therefore, the CNN
          analyze the spectrogram considering it as an image. Below you can inspect some of the
          features used:
        </ProjectParagraph>
        <FeaturesAccordion />
      </ProjectSection>
      <ProjectSection key="ds-header" id="datasets-header" title="Datasets">
        ...
        <ProjectParagraph>
          ...
          and data trustworthy issues. Additionally, the bitrate of the files was reduced from 32 to
          16, optimizing the time to train on the available hardware.
        </ProjectParagraph>
      </ProjectSection>
      <ProjectSection key="eval-header" id="eval-header" title="Performance Evaluation">
        <ProjectParagraph>
          ...
          were calculated in short-term using a window size of 1024, a 256 hop, and a
      ...
      <ProjectSection key="exp-header" id="exp-header" title="Experimental Results">
        <ProjectParagraph>
          ...
          dataset gave better models than the Coughvid. The Coswara SVM achieved the best precision
          0.77, recall 0.72, and F1-score 0.72 while not under-performing in other metrics.
        </ProjectParagraph>
        <Col>
          <Row>
            <ResultsTable />
          ...
          <Row>
            <ProjectParagraph>
              The SVM models had a better overall performance when compared to CNN. Coswara SVM had
              the best results; below is the confusion matrix and the ROC curve of the classifier.
              Especially the Coswara results show the best curve, in which we can achieve almost 70%
              true positives with around 25% false positives.
            </ProjectParagraph>
            <ResultSVMTabs />
          </Row>
          <Row>
            <ProjectParagraph>
              ...
              confusion matrix, we can analyze that its results mainly predict non-COVID relations,
              which is why we should consider more than just accuracy with unbalanced datasets.
              Finally, the Coughvid CNN presented a ROC very near the random guessing line, which
              could indicate that the model had problems fitting the data.
            </ProjectParagraph>
            <ResultCNNTabs />
          </Row>
        </Col>
      ...
      <ProjectSection key="conc-header" id="conc-header" title="Conclusion">
        <ProjectParagraph>
          ...
          when compared to recent literature. One of the possible reasons for these results is that
          cough information alone is insufficient for a precise classification. Experimental results
          also favored the traditional audio features techniques with an SVM compared to the CNN
          approach, which can be related to the simplicity of the employed CNN. Using a CNN with
          transfer learning might present better results.
        </ProjectParagraph>
        <ProjectParagraph>
          ...
          time. Additionally, future work could explore a mixed approach classification, considering
          both the audio file and other related symptoms features.
        </ProjectParagraph>
      </ProjectSection>
