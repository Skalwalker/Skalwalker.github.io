      ...
      <ProjectPageContainer project={page_project} footer={footer_info}>
        <ProjectSection id="intro-header" title="Introduction" first={true}>
          <ProjectParagraph>
            ...
            are guided by some form of prediction or analysis, such as studying the price's behavior
            or reading the company's economic report.
          </ProjectParagraph>
          <ProjectParagraph>
            This work supports the hypothesis that it is possible to model a high-accuracy service
            to predict the stock's movement and trade market assets based on technical and
        ...
        <ProjectSection id="overview-header" title="Overview">
          <ProjectParagraph>
            ...
            predict a given asset's value. On the other hand, the fundamental variables observe the
            existence of possible risks in the asset. Such information is used as input to a deep
            reinforcement learning model to manage the investor's resources in his portfolio. As a
            result, AURORA was designed into three modules: (i) the <b>Insider</b>, a predictor
            module that can predict the stocks' movements; (ii) a <b>risk management module</b> to
            deal with market volatility through the news and (iii) an <b>agent module</b>, a
            rational actuator that aims to allocate the resources available to the agent and act
            upon them. The Figure below shows an overview of AURORA's service and operation.
          </ProjectParagraph>
          <Figure className="mx-auto text-center">
            ...
            <Figure.Caption className="paragraph">
              ...
              interaction. The Figure highlights the Agent Module and its submodules
            </Figure.Caption>
          </Figure>
          <AuroraModules />
        </ProjectSection>
        <ProjectSection id="method-header" title="Environment">
          <ProjectParagraph>
            ...
            environment simulating the B3 stock market. This environment is designed for training
            and testing the DDPG model and defines the state space, action space, and reward
            function. The following definitions were used:
          </ProjectParagraph>
        ...
        <ProjectSection id="method-header" title="Methodology">
          <ProjectParagraph>
            The dataset used to feed the service of AURORA was obtained manually from the B3
            official website<sup>2</sup>. The website provides daily information from assets since
            1986, such as company name and code, stock code, market type, pricing, number of trades,
            and volume traded with paper. AURORA's model uses a semester time window for training,
            validation, and testing. In our approach, the agent's portfolio comprised the VALE3,
            PETR3, and ABEV3 assets. We made predictions on the assets' closing price as it reflects
            the day's movements in the market. The label of our data was created using a discrete
            approach for each asset at each time step, indicating whether an asset will gain or lose
            value.
          </ProjectParagraph>
          <ProjectParagraph>
            ...
            correctly. Second, we uncovered whether the DDPG ResAM would be able to handle a
            portfolio allocation, given that it knows when stock will increase or decrease in value.
          </ProjectParagraph>
        </ProjectSection>
        <ProjectSection id="dev-header" title="Stock Prediction">
          <ProjectParagraph>
            Concerning the ability to predict the movement of a stock, the objective was to
            determine if it was feasible to model an LSTM in a decision system like AURORA. Each
            stock was trained using the hyper parameterization process's best settings<sup>3</sup>.
            The experiment consisted of K-fold cross-validation, with k = 6, for multiple window
            sizes.
          </ProjectParagraph>
          <ResultsTabs />
        </ProjectSection>
        <ProjectSection id="res-header" title="Portfolio Allocation">
          <ProjectParagraph>
            ...
            was to prove that AURORA could manage a portfolio with multiple assets in a stock
            market. Therefore, we compared AURORA with the Savings Profitability and a buy and hold
            strategy<sup>4</sup>. The table below presents the parameters established to carry out
            the experiments. We used the following performance measures to evaluate the efficiency
            of AURORA: profit, profitability, return and yield.
          </ProjectParagraph>
          <RLTable />
          <ProjectParagraph>
            We built our portfolio with the PETR3, VALE3, and ABEV3 assets and with the parameters
            of the table above. The agent's initial investment money was defined as R$ 50,000.00,
            allowing for multiple investment possibilities. The figure below shows the behavior of
            the average return during the experiments.
          </ProjectParagraph>
          <Figure className="mx-auto text-center">
            <Figure.Image
              className="w-80"
          ...
          <ProjectParagraph>
            ...
            portfolio created was based on the Markowitz Mean-Variance theory [3]. It states that
            the construction process of an optimal portfolio considers the quantification of each
            asset's profit and risk with the objective of creating a frontier of the most efficient
            portfolios [3]. This frontier is presented with the set of portfolios that theoretically
            provides the higher return with the lowest risk. Markowitz's efficient frontier for the
            selected asset composition is shown in the figure below. The color of each portfolio
            combination is related to an indicator that helps to find a balance point between risk
            and return, called Sharpe ratio [4]. We then calculated the efficient frontier with a
            set of 25,000 random simulations. For the comparison, we considered two portfolios with
            the lowest risk and the highest Sharpe ratio, represented by the green and red markers.
            The lowest risk portfolio estimates a half-yearly return of 1.18%, with a risk of 45%.
            On the other hand, Sharpe's highest ratio portfolio has a potential return of 4.28% with
            a risk of 80%.
          </ProjectParagraph>
          <Figure className="mx-auto text-center">
            <Figure.Image
          ...
          <ProjectParagraph>
            ...
            return of 11.74% more significant than the actual and projected returns of the
            portfolios considered efficient. The yield obtained is also 250.44% greater than the
            savings yield.
          </ProjectParagraph>
          <ResultsTable />
        </ProjectSection>
        <ProjectSection id="conc-header" title="Conclusion">
          <ProjectParagraph>
            Experimental results show that the proposed service can predict the gain or loss of
            value at the price of a stock with an accuracy higher than 82.86% in the worst case and
            89.23% in the best case. Furthermore, the proposed service can achieve profitability of
            11.74%, overcoming fixed-income investments and portfolios built with the Markowitz
            Mean-Variance model. These results corroborate the hypothesis that AURORA is a reliable
            service to trade assets in the stock market with high precision and stability.
          </ProjectParagraph>
          <ProjectParagraph>
            ...
            hyper-parameterization of the DDPG using Hyperopt, experimenting with more input
            features for the LSTM model, studying possible reformulations of the B3 environment, and
        ...
        <Accordion.Item eventKey="1">
          ...
          <Accordion.Body className="text-left">
            <ProjectParagraph>
              ...
              volatility about external influences, the RMM was modeled into AURORA. The RMM's goal
              is to make a fundamental analysis using multiple sources of information to reflect an
        ...
        <Accordion.Item eventKey="2">
          ...
          <Accordion.Body className="text-left">
            ...
            <ProjectParagraph>
              ...
              liability that could place the agent's profit in jeopardy. After learning, the model
              decides which asset should be sold or bought at each step. AURORA's ResAM was modeled
              based on a deep reinforcement learning model, denominated Deep Deterministic Policy
              Gradient (DDPG). In such, the entity will need to learn how to map the market's state
              with responsive actions, aiming to increase its profits and reduce risks.
            </ProjectParagraph>
          </Accordion.Body>
        </Accordion.Item>
      </Accordion>
    ...
    <Container>
      ...
      >
        <Tab eventKey="best_case" title="Prediction Plots" className="w-100 text-left">
          <ProjectParagraph>
            We analyzed how AURORA's LSTM model behaved in an actual market situation. The goal was
            to understand the behavior of the predictions over the days. As can be seen in the
            ...
            stock's continuous movements, the modeled LSTM showed great accuracy and ensured that
            its previous errors did not affect its performance.
          </ProjectParagraph>
          <Figure className="mx-auto text-center">
          ...
          <ProjectParagraph>
            The plots show the predictions made by AURORA's LSTM model for each stock. The cyan line
            represents the average price in specific time steps. The blue line is the closing price
            of the stock on that day. The markers represent the correctness of the predictions.
          </ProjectParagraph>
        </Tab>
