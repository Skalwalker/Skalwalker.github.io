      ...
      <ProjectPageContainer project={page_project} footer={footer_info}>
        <ProjectSection id="intro-header" title="Introduction" variant="first">
          <ProjectParagraph>
            ...
            The core of his work strategy relies on using a mixture of alpha-stable motions
            modulated by the scene's complexity. This strategy allows exploiting the characteristics
            of long-tailed distributions of gaze shift lengths for analyzing dynamic scenes. The
            work modules the scan-paths as a Langevin-like stochastic differential equation [1]
            whose noise is sampled from the distributions to the composition of Lévy flights random
            walks [1]. Thus it can treat different types of eye movement (smooth pursuit, saccades,
            fixational movements) within the same framework.
          </ProjectParagraph>
        ...
        <ProjectSection id="dev-header" title="Overview">
          <ProjectParagraph>
            ...
            sampling algorithms or multiple internal mechanisms. A developer should be able to
            easily adapt the code to change, e.g., the salience map generation or the feature map
            algorithm. Additionally, a configuration file with multiple parameters for experiment
            execution is provided to support the execution of experiments with ecological sampling.
            Finally, for experienced users, comprehensive in-code and website documentation is
            provided, thus allowing a better understanding of the system and a deeper understanding
          ...
          <ProjectParagraph>
            ...
            passed for the FeatureMap class, which uses the backend class Seo's SelfRessemblance [4]
            to calculate the 3D LARK descriptors. Further, the same backend supports the SalienceMap
            class, which inputs the LARK features and outputs a static and space-time visual
            saliency. The ProtoParameters class can compute a patch map and identify proto-objects
            given the salience map. Following the PyEcosampling IPSampler class, the salience map or
            the proto-objects can be used to sample interest points. Those interest points are
            represented as a histogram of their empirical distributions, which is used for the
            Complexity class to calculate the landscape order, disorder, and complexity values. With
            such values, the ActionSampler can sample the appropriate gaze motion for the frame.
        ...
        <ProjectSection id="tec-header" title="Technologies">
          <ProjectParagraph>
            The PyEcosampling library relies on Python 3.8 and Tkinter, a GUI backend for
            matplotlib. Regarding the operational system. PyEcosampling was tested on macOS Monterey
            (version 12.4) and Ubuntu 20. A sample sequence for demo purposes is provided with the
            source code. Additionally, PyEcosampling supports adding your clips as a sequence of
            ordered frames.
          </ProjectParagraph>
          <ProjectParagraph>
            PyEcosampling might be used as an application stand-alone or as an API to write your
            gaze sampling script. It should be enough to generate all the processes of gaze sampling
            with interactive visualization and data saving for your application. You can obtain
            different gaze-shifting behaviors by playing with parameters in the configuration
            script. If you want an in-depth understanding of how the project is organized or the
            functioning of the API, one documentation was developed using sphinx. This documentation
            was developed knowing that if the software is to be published as a pip package, the
            documentation is ready to be uploaded to Read the Docs.
          </ProjectParagraph>
        </ProjectSection>
        <ProjectSection id="res-header" title="Performance Evaluation">
          <ProjectParagraph>
            The Ecosampling's original system was implemented in plain MATLAB code with no specific
            optimizations and executed on a 2.8 GHz Intel Core 2 Duo processor, 4 GB RAM, under Mac
            OS X 10.5.81. The original paper calculates the execution time of the experiment in
            seconds per frame (spf) measurement. Regarding actual performance under such a setting,
          </ProjectParagraph>
          <ProjectParagraph>
            ...
            higher than in MATLAB, while only the feature computation was of significant impact,
            1.925 to 1.155. Still, both implementations have the same time complexity.
          </ProjectParagraph>
        </ProjectSection>
        <ProjectSection id="conc-header" title="Conclusion">
          <ProjectParagraph>
            ...
            considering that Ecosampling is developed in Python, with complete documentation, use
            case examples, and even porting the Seo's self-resemblance work [4], hopefully, this
          ...
          <ProjectParagraph>
            ...
            processing techniques, possibly implementing a vectorized approach. Additionally, if
            this software is to be disclosed publicly, implementing command line execution and
            publishing the software in the Python Package Index (PyPI) could be of great interest.
          </ProjectParagraph>
        </ProjectSection>
        ...
        <Accordion.Item eventKey="1">
          ...
          <Accordion.Body className="text-left">
            <ProjectParagraph>
              ...
              PyEcosampling version, the FeatureMap class calls Seo's 3D-LARK description features
              to reproduce the static and space-time visual saliency by self-resemblance. The
              3D-LARK descriptor measures a self-similarity based on the "signal-induced distance"
